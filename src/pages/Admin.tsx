        ...
        <section className="overflow-hidden rounded-[2rem] border border-caramel/20 bg-white/85 shadow-warm backdrop-blur">
          <div className="border-b border-caramel/10 bg-gradient-to-r from-butter/45 via-white to-caramel/10 p-4">
            <div className="flex items-center gap-3">
              <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-caramel text-white shadow-sm"><currentTab.icon className="h-6 w-6" /></div>
...
const AdminLogin = ({ password, setPassword, authError, isLoading, onLogin }: { password: string; setPassword: (value: string) => void; authError: string; isLoading: boolean; onLogin: () => void }) => (
  <div className="flex min-h-screen items-center justify-center bg-gradient-to-b from-[#23140f] via-[hsl(35_45%_92%)] to-background px-4 pb-24 pt-20"><motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-sm"><div className="relative overflow-hidden rounded-[2rem] border border-white/10 bg-gradient-to-br from-espresso via-[#2b1811] to-caramel p-5 text-white shadow-elevated"><div className="absolute -right-14 -top-14 h-40 w-40 rounded-full bg-white/10 blur-sm" /><div className="relative text-center"><div className="mx-auto mb-5 flex h-16 w-16 items-center justify-center rounded-3xl bg-white/12 text-butter"><Lock className="h-8 w-8" /></div><p className="text-xs font-bold uppercase tracking-[0.18em] text-white/70">Accès personnel</p><h1 className="mt-1 font-display text-3xl font-black">Panel Admin</h1><p className="mt-2 text-sm leading-relaxed text-white/78">Poste de pilotage privé de La Crêperie des Saveurs.</p></div></div><div className="mt-4 rounded-[2rem] border border-caramel/15 bg-white/85 p-5 shadow-warm backdrop-blur"><div className="mb-4 flex items-start gap-3 rounded-2xl bg-caramel/10 p-3"><ShieldCheck className="mt-0.5 h-5 w-5 shrink-0 text-caramel" /><p className="text-xs leading-relaxed text-muted-foreground">Le mot de passe est vérifié par Supabase. La session admin n’est pas persistée.</p></div><div className="space-y-4"><div className="space-y-2"><Label htmlFor="password">Mot de passe admin</Label><Input id="password" type="password" value={password} onChange={(e) => setPassword(e.target.value)} onKeyDown={(e) => e.key === "Enter" && onLogin()} placeholder="••••••••" className={`h-12 rounded-2xl ${authError ? "border-destructive" : ""}`} />{authError && <p className="text-xs font-semibold text-destructive">{authError}</p>}</div><Button onClick={onLogin} className="h-12 w-full rounded-2xl bg-caramel font-black text-white hover:bg-caramel/90" disabled={isLoading || !password}>{isLoading ? <Loader2 className="h-5 w-5 animate-spin" /> : "Connexion admin"}</Button></div></div></motion.div></div>
);

const AdminHeader = ({ currentTab, onLogout }: { currentTab: { group: AdminGroup; label: string; icon: LucideIcon }; onLogout: () => void }) => { const Icon = currentTab.icon; return <section className="relative overflow-hidden rounded-[2rem] border border-white/10 bg-gradient-to-br from-espresso via-[#2b1811] to-caramel p-5 text-white shadow-elevated"><div className="absolute -right-14 -top-14 h-40 w-40 rounded-full bg-white/10 blur-sm" /><div className="absolute -bottom-16 -left-16 h-44 w-44 rounded-full bg-butter/10 blur-sm" /><div className="relative flex items-start justify-between gap-4"><div><div className="mb-4 inline-flex items-center gap-2 rounded-full border border-white/20 bg-white/10 px-3 py-1.5 text-xs font-bold uppercase tracking-[0.18em] text-white/90 backdrop-blur"><LayoutDashboard className="h-3.5 w-3.5 text-butter" />Panel Admin</div><h1 className="font-display text-3xl font-black leading-tight">Gestion premium</h1><p className="mt-2 text-sm leading-relaxed text-white/78">Module actif : {currentTab.group} · {currentTab.label}</p></div><div className="flex gap-2"><div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-white/12 text-butter"><Icon className="h-6 w-6" /></div><button type="button" onClick={onLogout} className="rounded-2xl border border-white/20 bg-white/10 p-3 text-white/85 transition hover:bg-white/15" aria-label="Déconnexion admin"><LogOut className="h-5 w-5" /></button></div></div></section>; };

const AdminNavigation = ({ activeTab, setActiveTab }: { activeTab: AdminTab; setActiveTab: (tab: AdminTab) => void }) => <section className="rounded-[2rem] border border-caramel/15 bg-white/85 p-3 shadow-warm backdrop-blur"><div className="space-y-4">{ADMIN_GROUPS.map((group) => <div key={group}><p className="mb-2 px-1 text-[10px] font-black uppercase tracking-[0.18em] text-muted-foreground">{group}</p><div className="grid grid-cols-2 gap-2">{ADMIN_TABS.filter((tab) => tab.group === group).map((tab) => <AdminNavButton key={tab.id} tab={tab} active={activeTab === tab.id} onClick={() => setActiveTab(tab.id)} />)}</div></div>)}</div></section>;
...
const ValidationCameraCard = ({ scannerActive, onScannerToggle, onVerify }: { scannerActive: boolean; onScannerToggle: () => void; onVerify: (code: string) => void }) => <div className="rounded-3xl border border-border/55 bg-background/70 p-4"><div className="mb-4 flex items-center justify-between gap-3"><Label className="flex items-center gap-2 font-bold"><Camera className="h-4 w-4 text-caramel" />Contrôle caméra</Label><Button variant={scannerActive ? "destructive" : "outline"} size="sm" onClick={onScannerToggle} className="rounded-2xl">{scannerActive ? <><CameraOff className="mr-1 h-4 w-4" />Arrêter</> : <><Camera className="mr-1 h-4 w-4" />Activer</>}</Button></div>{scannerActive ? <CodeCamera onScan={onVerify} isActive={scannerActive} /> : <div className="flex aspect-square flex-col items-center justify-center rounded-3xl border border-dashed border-caramel/25 bg-butter/20 text-muted-foreground"><TicketCheck className="mb-3 h-12 w-12 opacity-35" /><p className="text-sm font-semibold">Activez la caméra pour valider un coupon</p></div>}</div>;
