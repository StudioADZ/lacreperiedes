...
const Admin = () => {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (error || !data.session?.user?.id) throw error ?? new Error("Connexion impossible");
      ...
      if (roleError || !hasAdminRole) {
        ...
        setAuthError("Ce compte ne possède pas le rôle administrateur.");
        return;
      }
  ...
  if (!adminToken) {
    return <AdminLogin email={email} password={password} setEmail={setEmail} setPassword={setPassword} authError={authError} isLoading={authLoading} onLogin={handleLogin} />;
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#23140f] via-[hsl(35_45%_92%)] to-background px-4 pb-24 pt-20">
      <div className="mx-auto max-w-xl space-y-5">
        <AdminHeader currentTab={currentTab} adminEmail={adminEmail} onLogout={handleLogout} />
...
const AdminLogin = ({ email, password, setEmail, setPassword, authError, isLoading, onLogin }: { email: string; password: string; setEmail: (value: string) => void; setPassword: (value: string) => void; authError: string; isLoading: boolean; onLogin: () => void }) => (
  <main className="flex min-h-screen items-center justify-center bg-gradient-to-b from-[#23140f] via-[hsl(35_45%_92%)] to-background px-4 pb-24 pt-20">
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-sm">
      <div className="rounded-[2rem] bg-gradient-to-br from-espresso via-[#2b1811] to-caramel p-5 text-center text-white shadow-elevated">
        <div className="mx-auto mb-5 flex h-16 w-16 items-center justify-center rounded-3xl bg-white/12 text-butter"><Lock className="h-8 w-8" aria-hidden="true" /></div>
        <p className="text-xs font-bold uppercase tracking-[0.18em] text-white/70">Accès nominatif sécurisé</p>
        <h1 className="mt-1 font-display text-3xl font-black">Administration</h1>
        <p className="mt-2 text-sm text-white/78">Connectez-vous avec votre compte autorisé. Le rôle administrateur est contrôlé par Supabase.</p>
      </div>
      <div className="mt-4 rounded-[2rem] border border-caramel/15 bg-white/90 p-5 shadow-warm">
        <div className="mb-4 flex items-start gap-3 rounded-2xl bg-caramel/10 p-3"><ShieldCheck className="mt-0.5 h-5 w-5 shrink-0 text-caramel" aria-hidden="true" /><p className="text-xs text-muted-foreground">Le mot de passe partagé n’est plus utilisé. Chaque action est liée au compte connecté.</p></div>
        <form onSubmit={(event) => { event.preventDefault(); void onLogin(); }} className="space-y-4">
          <div className="space-y-2"><Label htmlFor="admin-email">Email administrateur</Label><Input id="admin-email" type="email" autoComplete="username" value={email} onChange={(event) => setEmail(event.target.value)} className="h-12 rounded-2xl" required /></div>
          <div className="space-y-2"><Label htmlFor="admin-password">Mot de passe</Label><Input id="admin-password" type="password" autoComplete="current-password" value={password} onChange={(event) => setPassword(event.target.value)} className={`h-12 rounded-2xl ${authError ? "border-destructive" : ""}`} required />{authError && <p className="text-xs font-semibold text-destructive">{authError}</p>}</div>
          <Button type="submit" className="h-12 w-full rounded-2xl bg-caramel font-black text-white" disabled={isLoading || !email.trim() || !password}>{isLoading ? <Loader2 className="h-5 w-5 animate-spin" /> : "Connexion sécurisée"}</Button>
        </form>
      </div>
    </motion.div>
  </main>
