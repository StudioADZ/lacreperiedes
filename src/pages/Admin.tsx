  ...
  if (authLoading) {
    return (
      <main className="flex min-h-screen items-center justify-center" role="status">
        <Loader2 className="h-9 w-9 animate-spin text-caramel" />
        <span className="sr-only">Vérification de l’accès administrateur</span>
      </main>
    );
  }

  if (!isSignedIn) {
    return <AdminLogin authError={authError} isLoading={authLoading} onGoogleLogin={handleGoogleLogin} />;
  }

  if (!adminToken) {
    return <AccessDenied email={accountEmail} onLogout={handleLogout} />;
  }
          ...
          <div className="border-b border-caramel/10 bg-gradient-to-r from-butter/45 via-white to-caramel/10 p-4">
            <div className="flex items-center gap-3">
              <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-caramel text-white">
                <currentTab.icon className="h-6 w-6" aria-hidden="true" />
              </div>
              <div>
...
const AdminLogin = ({
  ...
  <main className="flex min-h-screen items-center justify-center bg-gradient-to-b from-[#23140f] via-[hsl(35_45%_92%)] to-background px-4 pb-24 pt-20">
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-sm">
      <div className="rounded-[2rem] bg-gradient-to-br from-espresso via-[#2b1811] to-caramel p-6 text-center text-white shadow-elevated">
        <div className="mx-auto mb-5 flex h-16 w-16 items-center justify-center rounded-3xl bg-white/12 text-butter">
          <Lock className="h-8 w-8" aria-hidden="true" />
        </div>
        <p className="text-xs font-bold uppercase tracking-[0.18em] text-white/70">Espace privé</p>
        <h1 className="mt-1 font-display text-3xl font-black">Administration</h1>
        <p className="mt-2 text-sm leading-relaxed text-white/78">Cet espace est réservé à l’équipe autorisée de La Crêperie des Saveurs.</p>
      </div>

      <div className="mt-4 rounded-[2rem] border border-caramel/15 bg-white/95 p-5 shadow-warm">
        <div className="mb-5 flex items-start gap-3 rounded-2xl bg-caramel/10 p-3">
          <ShieldCheck className="mt-0.5 h-5 w-5 shrink-0 text-caramel" aria-hidden="true" />
          <p className="text-xs leading-relaxed text-muted-foreground">Connexion sécurisée avec Google. Aucun mot de passe administrateur n’est demandé sur cette page.</p>
        </div>

        <Button type="button" onClick={onGoogleLogin} disabled={isLoading} className="h-13 w-full rounded-2xl bg-caramel font-black text-white hover:bg-caramel/90">
          {isLoading ? <Loader2 className="h-5 w-5 animate-spin" /> : <><LogIn className="mr-2 h-5 w-5" />Se connecter avec Google</>}
        </Button>
        ...
        <Button asChild variant="outline" className="mt-3 h-12 w-full rounded-2xl font-bold">
          <Link to="/"><Home className="mr-2 h-4 w-4" />Retour au site</Link>
        </Button>
      </div>
    </motion.div>
  </main>
...
const AccessDenied = ({ email, onLogout }: { email: string | null; onLogout: () => void }) => (
  <main className="flex min-h-screen items-center justify-center bg-gradient-to-b from-[#23140f] via-[hsl(35_45%_92%)] to-background px-4 pb-24 pt-20">
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-sm rounded-[2rem] border border-caramel/15 bg-white/95 p-6 text-center shadow-warm">
      <div className="mx-auto mb-5 flex h-16 w-16 items-center justify-center rounded-3xl bg-destructive/10 text-destructive">
        <XCircle className="h-8 w-8" aria-hidden="true" />
      </div>
      <p className="text-xs font-black uppercase tracking-[0.18em] text-caramel">Accès refusé</p>
      <h1 className="mt-2 font-display text-3xl font-black text-espresso">Compte non autorisé</h1>
      <p className="mt-3 text-sm leading-relaxed text-muted-foreground">Le compte {email ? <strong>{email}</strong> : "connecté"} ne possède pas le rôle administrateur.</p>
      <Button type="button" onClick={onLogout} className="mt-6 h-12 w-full rounded-2xl bg-caramel font-black text-white">Changer de compte</Button>
      <Button asChild variant="outline" className="mt-3 h-12 w-full rounded-2xl font-bold"><Link to="/"><Home className="mr-2 h-4 w-4" />Retour au site</Link></Button>
    </motion.div>
  </main>
);
...
const AdminHeader = ({ currentTab, adminEmail, onLogout }: { currentTab: { group: AdminGroup; label: string; icon: LucideIcon }; adminEmail: string | null; onLogout: () => void }) => {
  ...
  return (
    <section className="rounded-[2rem] bg-gradient-to-br from-espresso via-[#2b1811] to-caramel p-5 text-white shadow-elevated">
      <div className="flex items-start justify-between gap-4">
        <div>
          <div className="mb-4 inline-flex items-center gap-2 rounded-full border border-white/20 bg-white/10 px-3 py-1.5 text-xs font-bold uppercase tracking-[0.18em]"><LayoutDashboard className="h-3.5 w-3.5 text-butter" aria-hidden="true" />Panel Admin</div>
          <h1 className="font-display text-3xl font-black">Gestion premium</h1>
          <p className="mt-2 text-sm text-white/78">{currentTab.group} · {currentTab.label}</p>
          <p className="mt-1 text-xs text-white/55">{adminEmail}</p>
        </div>
        <div className="flex gap-2">
          <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-white/12 text-butter"><Icon className="h-6 w-6" aria-hidden="true" /></div>
          <button type="button" onClick={onLogout} className="rounded-2xl border border-white/20 bg-white/10 p-3" aria-label="Déconnexion administrateur"><LogOut className="h-5 w-5" /></button>
        </div>
      </div>
    </section>
  );
...
const AdminNavigation = ({ activeTab, setActiveTab }: { activeTab: AdminTab; setActiveTab: (tab: AdminTab) => void }) => (
  <section className="rounded-[2rem] border border-caramel/15 bg-white/85 p-3 shadow-warm">
        ...
        <div key={group}>
          ...
          <div className="grid grid-cols-2 gap-2">
            {ADMIN_TABS.filter((tab) => tab.group === group).map((tab) => {
              ...
              return (
                <button key={tab.id} type="button" onClick={() => setActiveTab(tab.id)} className={`rounded-2xl border p-3 text-left ${active ? "border-caramel bg-caramel text-white" : "border-border/60 bg-background/75 text-espresso"}`}>
                  <div className={`mb-2 flex h-9 w-9 items-center justify-center rounded-xl ${active ? "bg-white/18" : "bg-caramel/10 text-caramel"}`}><Icon className="h-4 w-4" aria-hidden="true" /></div>
                  <p className="font-display text-sm font-black">{tab.label}</p>
                  <p className={`mt-0.5 text-[10px] ${active ? "text-white/72" : "text-muted-foreground"}`}>{tab.description}</p>
                </button>
              );
            })}
          </div>
...
const ValidationPanel = ({ result, manualCode, isLoading, claimLoading, onManualCodeChange, onVerify, onClaim, onReset }: { result: VerifyResult | null; manualCode: string; isLoading: boolean; claimLoading: boolean; onManualCodeChange: (code: string) => void; onVerify: (code: string) => void; onClaim: () => void; onReset: () => void }) => (
  <div className="space-y-5">
    {!result && <div className="rounded-3xl border border-border/55 bg-background/70 p-4"><Label htmlFor="code" className="mb-2 flex items-center gap-2 font-bold"><TicketCheck className="h-4 w-4 text-caramel" aria-hidden="true" />Saisie manuelle</Label><div className="flex gap-2"><Input id="code" value={manualCode} onChange={(event) => onManualCodeChange(event.target.value.toUpperCase())} onKeyDown={(event) => event.key === "Enter" && onVerify(manualCode)} placeholder="XXXXXXXX" className="h-12 rounded-2xl font-mono text-lg tracking-wider" maxLength={8} /><Button onClick={() => onVerify(manualCode)} disabled={isLoading || !manualCode.trim()} className="h-12 rounded-2xl bg-caramel font-bold text-white">{isLoading ? <Loader2 className="h-5 w-5 animate-spin" /> : "Contrôler"}</Button></div></div>}
    <AnimatePresence mode="wait">{result && <ValidationResult result={result} claimLoading={claimLoading} onClaim={onClaim} onReset={onReset} />}</AnimatePresence>
  </div>
);
...
const ValidationResult = ({ result, claimLoading, onClaim, onReset }: { result: VerifyResult; claimLoading: boolean; onClaim: () => void; onReset: () => void }) => {
  ...
  return (
    <motion.div initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} className={`rounded-[2rem] border p-6 text-center ${usable ? "border-herb/35 bg-herb/10" : result.valid ? "border-border/70 bg-muted/35" : "border-destructive/25 bg-destructive/10"}`}>
      <div className="mx-auto mb-5 flex h-20 w-20 items-center justify-center rounded-full bg-white/70">{usable ? <CheckCircle className="h-10 w-10 text-herb" /> : result.valid ? <AlertCircle className="h-10 w-10 text-muted-foreground" /> : <XCircle className="h-10 w-10 text-destructive" />}</div>
      ...
      <Button variant="outline" onClick={onReset} className="mt-5 rounded-2xl font-bold">Nouveau contrôle</Button>
    </motion.div>
  );
};
