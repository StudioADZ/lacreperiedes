...
type AdminTab = "scan" | "messages" | "quiz" | "carte" | "actus" | "payment" | "splash";
type AdminGroup = "Caisse" | "Pilotage" | "Contenu" | "Réglages";
...
const ADMIN_TABS: { id: AdminTab; group: AdminGroup; label: string; description: string; icon: LucideIcon }[] = [
  ...
  { id: "quiz", group: "Pilotage", label: "Quiz", description: "Stats et lots", icon: BarChart3 },
...
const ADMIN_GROUPS: AdminGroup[] = ["Caisse", "Pilotage", "Contenu", "Réglages"];
    ...
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-sm">
      ...
      <div className="mt-4 rounded-[2rem] border border-caramel/15 bg-white/85 p-5 shadow-warm backdrop-blur">
        ...
        <div className="space-y-4">
          ...
          <Button onClick={onLogin} className="h-12 w-full rounded-2xl bg-caramel font-black text-white hover:bg-caramel/90" disabled={isLoading || !password}>
    ...
    <section className="relative overflow-hidden rounded-[2rem] border border-white/10 bg-gradient-to-br from-espresso via-[#2b1811] to-caramel p-5 text-white shadow-elevated">
      ...
      <div className="relative flex items-start justify-between gap-4">
        ...
        <div className="flex gap-2">
          <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-white/12 text-butter">
        ...
        <div key={group}>
          ...
          <div className="grid grid-cols-2 gap-2">
            {ADMIN_TABS.filter((tab) => tab.group === group).map((tab) => (
              <AdminNavButton key={tab.id} tab={tab} active={activeTab === tab.id} onClick={() => setActiveTab(tab.id)} />
            ))}
          </div>
        </div>
      ))}
...
const AdminNavButton = ({
  ...
  tab: { id: AdminTab; label: string; description: string; icon: LucideIcon };
  active: boolean;
  onClick: () => void;
}) => {
  const Icon = tab.icon;
    ...
    <button
      ...
      className={`rounded-2xl border p-3 text-left transition ${
        active
          ...
          : "border-border/60 bg-background/75 text-espresso hover:border-caramel/40 hover:bg-white"
      }`}
    >
      <div className={`mb-2 flex h-9 w-9 items-center justify-center rounded-xl ${active ? "bg-white/18 text-white" : "bg-caramel/10 text-caramel"}`}>
  ...
  return (
    <motion.div
      ...
      className={`rounded-[2rem] border p-6 text-center shadow-sm ${
        isValidUnused ? "border-herb/35 bg-herb/10" : isClaimed ? "border-border/70 bg-muted/35" : "border-destructive/25 bg-destructive/10"
      }`}
    >
      <motion.div
        initial={{ scale: 0 }}
        ...
        className={`mx-auto mb-5 flex h-20 w-20 items-center justify-center rounded-full ${isValidUnused ? "bg-herb/10" : isClaimed ? "bg-background" : "bg-destructive/10"}`}
      >
        {isValidUnused ? <CheckCircle className="h-10 w-10 text-herb" /> : isClaimed ? <AlertCircle className="h-10 w-10 text-muted-foreground" /> : <XCircle className="h-10 w-10 text-destructive" />}
      </motion.div>
