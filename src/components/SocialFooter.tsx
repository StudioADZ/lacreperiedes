...
const SocialFooter = () => {
  ...
  if (location.pathname.startsWith("/reseaux") || location.pathname.startsWith("/social")) {
    return null;
  }
    ...
    <footer className="mt-12 pb-8">
      <div className="max-w-lg mx-auto px-4">
        ...
        <div className="text-center mb-6 p-4 rounded-2xl bg-gradient-to-r from-yellow-50 to-orange-50 border border-yellow-200">
          ...
          <p className="text-xs text-muted-foreground mt-2">Merci, ça aide énormément une petite crêperie locale. 💛</p>
        </div>
          ...
          {socialLinks.map((link) => (
            <a
              ...
              className={`w-12 h-12 rounded-full flex items-center justify-center shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-110 ${
                link.gradient ? `bg-gradient-to-br ${link.gradient}` : link.bgColor
              } ${"textColor" in link && link.textColor ? link.textColor : "text-white"}`}
              style={{
                boxShadow:
                  "0 4px 15px -3px rgba(0,0,0,0.2), 0 0 20px -5px rgba(255,200,100,0.3)",
              }}
            >
              {link.icon}
            </a>
