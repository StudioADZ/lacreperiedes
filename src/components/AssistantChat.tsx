...
const makeId = () => {
  // ✅ Safe id generator (no breaking changes)
  try {
...
const AssistantChat = () => {
  ...
  // ✅ Helper: register timeouts for cleanup
  ...
  // ✅ Cleanup pending timeouts on unmount (or hot reload)
  useEffect(() => {
    return () => {
  ...
  if (FORBIDDEN_ROUTES.some((r) => location.pathname.startsWith(r))) {
    return null;
  }
  ...
  // ✅ Safe external open (prevents opener access)
  ...
  const handleQuickAction = useCallback(
    (action: string) => {
      switch (action) {
        ...
        case "reserver":
          ...
          registerTimeout(() => {
            safeOpen(BOOKING_LINK);
          }, 1000);
          break;

        case "avis":
          addBotMessage(
            "⭐ Merci de vouloir nous laisser un avis ! Votre feedback nous aide à nous améliorer. Ça aide énormément une petite crêperie locale. 💛",
          );
          registerTimeout(() => {
            safeOpen(GOOGLE_REVIEW_LINK);
          }, 1000);
          break;

        case "trouver":
          // ✅ Removed markdown (was **bold**) -> safe plain text
          addBotMessage(
            "📍 Nous sommes situés au :\n\n17 Place Carnot\nGalerie des Halles\n72600 Mamers\n\nJe vous ouvre Google Maps !",
          );
          registerTimeout(() => {
            safeOpen("https://maps.app.goo.gl/6KdHfHSUs1MbzakLA");
          }, 1500);
          break;

        case "legal":
          addBotMessage("📜 Vous trouverez notre règlement, politique de confidentialité et conditions d'utilisation sur la page dédiée.");
          registerTimeout(() => navigate("/legal"), 1000);
          break;

        case "carte":
          addBotMessage("🍽️ Notre carte secrète vous réserve des surprises ! Participez au quiz pour débloquer l'accès.");
          registerTimeout(() => navigate("/carte"), 1000);
          break;
        ...
        case "whatsapp":
          ...
          registerTimeout(() => {
            safeOpen("https://wa.me/message/QVZO5N4ZDR64M1");
          }, 1000);
          break;
    ...
    [addBotMessage, navigate, registerTimeout, safeOpen],
  );
  ...
  const handleUserMessage = useCallback(
    (text: string) => {
      ...
      if (
        ...
        lowerText.includes("valider lot")
      ) {
        registerTimeout(() => {
      ...
      if (
        lowerText.includes("adresse") ||
        lowerText.includes("où") ||
        lowerText.includes("trouver") ||
        lowerText.includes("lieu") ||
        lowerText.includes("localisation")
      ) {
      ...
      if (
        ...
        lowerText.includes("jeu") ||
        lowerText.includes("gagner") ||
        lowerText.includes("jouer")
      ) {
      ...
      if (
        ...
        lowerText.includes("manger") ||
        lowerText.includes("crêpe") ||
        lowerText.includes("galette")
      ) {
      ...
      // Règlement / RGPD
      if (
        lowerText.includes("règlement") ||
        lowerText.includes("rgpd") ||
        lowerText.includes("confidentialité") ||
        lowerText.includes("condition") ||
        lowerText.includes("légal")
      ) {
