...
function writeCookie(name: string, value: string, maxAgeSeconds: number) {
  ...
  // Add Secure when on https (avoid breaking localhost/http)
  try {
...
>(({ defaultOpen = true, open: openProp, onOpenChange: setOpenProp, className, style, children, ...props }, ref) => {
  ...
  // Read persisted cookie only when uncontrolled
  React.useEffect(() => {
    if (openProp !== undefined) return; // controlled: don't override
    const saved = readCookie(SIDEBAR_COOKIE_NAME);
    if (saved === null) return;

    // Accept "true"/"false" only
    if (saved === "true") _setOpen(true);
  ...
  const setOpen = React.useCallback(
    (value: boolean | ((value: boolean) => boolean)) => {
      ...
      writeCookie(SIDEBAR_COOKIE_NAME, String(openState), SIDEBAR_COOKIE_MAX_AGE);
    },
    [setOpenProp, open],
  );
...
// ... (le reste du fichier est IDENTIQUE à ton code, non modifié)
...
// (ton code menu etc. inchangé)
const SidebarMenu = React.forwardRef<HTMLUListElement, React.ComponentProps<"ul">>(({ className, ...props }, ref) => (
  <ul ref={ref} data-sidebar="menu" className={cn("flex w-full min-w-0 flex-col gap-1", className)} {...props} />
));
SidebarMenu.displayName = "SidebarMenu";

// ... tout le reste pareil que ton fichier original ...
// IMPORTANT: colle ici la suite inchangée depuis ton fichier, ou garde ton fichier actuel
// et applique juste les changements SAFE ci-dessus.
